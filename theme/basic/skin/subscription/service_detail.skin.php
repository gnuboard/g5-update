...
?>
    <div class="item_box remove_border">
        <ul>
            <li class="title_area">
                <div class="service_name"><?= $service_info['name'] ?></div>
                <div class="service_summary"><?= $service_info['summary'] ?></div>
            </li>
            <li class="price_area">
                <div class="price"><?=$convertYMDUnit[$service_info['recurring_unit']] ?> <?= number_format($service_info['price']) ?>원 (부가세 포함)</div>
                <div><?=$service_info['recurring']?><?=$convertYMDUnit2[$service_info['recurring_unit']] ?> 마다 정기 결제</div>
            </li>
