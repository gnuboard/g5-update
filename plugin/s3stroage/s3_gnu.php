...
class AWS_S3_GNU5
{
    ...
    private $db_table = 'aws_s3_config';
    private $config;

    public $default_region = 'ap-northeast-2';  // aws s3 서울 지역
    public $storage_prefix = 'aws_s3';
    public $extra_item_field = 'aws_images';
    public $shop_folder = 'item';
    ...
    public function __construct()
    {
        ...
        if ($this->config['s3_bucket_name'] && $this->config['s3_bucket_region'] && $this->config['s3_user_key'] && $this->config['s3_user_secret']) {
            $this->add_hooks();
        }

        $this->extra_item_field = preg_replace('/[^a-z0-9_]/i', '', $this->extra_item_field);
        $this->config['region'] = $this->default_region;

        if ($this->config['s3_bucket_region'] && array_key_exists(
                $this->config['s3_bucket_region'],
                $this->get_regions()
            )) {
            $this->config['region'] = $this->config['s3_bucket_region'];
        }
    }
    ...
    public function get_config($is_cache = false)
    {
        ...
        $cache = run_replace('get_aws_s3_config_cache', $cache, $is_cache);
        ...
        $cache = run_replace('get_aws_s3_config', sql_fetch($sql, false));
        if (!$cache) {
            $sql = get_db_create_replace(
                "CREATE TABLE IF NOT EXISTS `$table_name` (
				  `s3_bucket_name` varchar(255) NOT NULL DEFAULT '',
				  `s3_bucket_region` varchar(255) NOT NULL DEFAULT '',
				  `s3_user_key` varchar(255) NOT NULL DEFAULT '',
				  `s3_user_secret` varchar(255) NOT NULL DEFAULT '',
				  `s3_access_control_list` varchar(255) NOT NULL DEFAULT '',
				  `s3_save_mydata` tinyint(4) NOT NULL DEFAULT '0',
				  `s3_only_used_file` tinyint(4) NOT NULL DEFAULT '0'
    ...
    {
        return $this->s3Client->getObjectUrl($bucket_name, $key);
    }
    ...
    {
        return $this->s3Client->copyObject($args);
    }

    public function get_paginator($action, $args)
    {
        return $this->s3Client->getPaginator($action, $args);
    }
    ...
    public function admin_page_config($arr_query, $token)
    {
        ...
        include_once(G5_PLUGIN_PATH . '/aws_s3/skin/adm.config.php');
    }
        ...
        for ($i = 1; $i <= 10; $i++) {
            if ($it['it_img' . $i]) {
                ...
                if (isset($matches[1]) && $this->aws_s3_url_validate($matches[1])) {
                    ...
                    if (isset($tmps[1]) && stripos($tmps[1], G5_URL) !== false && preg_match(
    ...
    public function shop_admin_itemformupdate($it_id, $w)
    {
        ...
        $merges = array_merge($img_arrays, (array)$before_infos);
    ...
    public function replace_aws_url($fileurl)
    {
        ...
        $aws_image_url = 'https://' . $this->config['s3_bucket_name'] . '.s3.' . $this->config['region'] . '.amazonaws.com' . str_replace(
                G5_URL,
                '',
                $fileurl
            );
    ...
    {
        $replace_url = 'https://' . $this->config['s3_bucket_name'] . '.s3.' . $this->config['region'] . '.amazonaws.com/' . G5_DATA_DIR;
    ...
    {
        // https://docs.aws.amazon.com/ko_kr/AmazonS3/latest/API/RESTBucketGET.html
        $image_url = 'https://' . $this->config['s3_bucket_name'] . '.s3.' . $this->config['region'] . '.amazonaws.com/' . $file_key;
        ...
        /* User agent to identify this image retriever as */
    ...
    // 에디터의 file url 이 aws s3 url 이 맞는지 체크
    ...
    {
        if (file_exists($filepath) && $this->s3_client()) {
    ...
    // 에디터에서 올린 파일 삭제시
    function delete_editor_file($file_path, $is_success = false)
    {
                ...
                if ($size && isset($board['bo_image_width']) && $size[0] > $board['bo_image_width']) {
                    ...
                    if ($thumb_file = thumbnail(
                        ...
                        $thumb_result = $this->put_object(array(
                            'Bucket' => $this->config['s3_bucket_name'],
                            'Key' => $thumb_key,
                            'Body' => fopen($thumb_path . '/' . $thumb_file, 'r'),
                            'ACL' => $this->file_acl($thumb_key),
                            'ContentType' => $upload_mime,
                        ));
