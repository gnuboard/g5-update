...
CREATE TABLE `g5_billing_service_price` (
  ...
  `application_end_date` datetime DEFAULT '0000-00-00 00:00:00',
  PRIMARY KEY (`id`),
...
CREATE TABLE `g5_billing_config` (
  ...
  `bc_kcp_cert` varchar(255) DEFAULT '' COMMENT 'kcp 서비스 인증서',
  `bc_kcp_prikey` varchar(255) DEFAULT '' COMMENT 'kcp 암호화 개인 키',
  ...
  `bc_toss_client_key` varchar(255) DEFAULT '' COMMENT 'toss 클라이언트 키',
  `bc_toss_secret_key` varchar(255) DEFAULT '' COMMENT 'toss 시크릿 키',
  `bc_toss_customer_key` varchar(255) DEFAULT '' COMMENT 'toss 고객 ID',
