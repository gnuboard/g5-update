...
$ct_id = isset($_REQUEST['ct_id']) ? preg_replace('/[^0-9]/', '', trim($_REQUEST['ct_id'])) : 0;
$use = array('is_subject' => '', 'is_content' => '');
$is_score = 5;

if ($ct_id) {
    $cart = sql_fetch("SELECT it_id FROM {$g5['g5_shop_cart_table']} WHERE ct_id = '{$ct_id}' AND mb_id = '{$member['mb_id']}'");
    $it_id = $cart['it_id'];
}
